    ...
    def handle_event(self, event):
        try:
            if not self.find_token_flag:
                ...
                method = transaction.input[:10].lower()
                to_address = transaction.to
                if (method in liquidity_add_methods or self.new_token_presale.lower() == to_address.lower()):
                    address = transaction.input[34:74]
                    address = "0x" + address
                    if (address.lower() == self.new_token.lower()):
